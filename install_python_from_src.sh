...
install_python_from_source() {
    ...
    # Requiring pymodule 'readline' would in turn require 'libncurses'
    ...
    if [[ -d "$tmpdir" ]]; then
        ...
        tar xvfz $python_src_file.tgz && cd $python_src_file   \
            && sudo ./configure --enable-shared --with-ensurepip=install \
                  --prefix=$tgt_dir && sudo make && sudo make install
    fi
}
